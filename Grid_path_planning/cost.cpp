...
void ExtractAstar(string astar) {//
   ...
   int num = 0;
      ...
      while(getline(file,line)){
	      if(isdigit(line[0])) {
		      int count = 0, forward_, backward_, OSME,chi,t1,t2,t3,t4;
		      string mapNum, mapNa, width, height, startX,startY,goalX,goalY;
		      double pathCost,times;	  
		      stringstream strr(line);//>> num >> mn
              strr >> mapNum >> mapNa>> width>> height>>startX >> startY >> goalX >> goalY >> pathCost>> forward_ >> backward_>> times >> OSME; 
...
void averageCost()
{
   ...
   double averOSME = (double) OpT_ / forwardE.size();
   cerr << averForward << " " << backwardE.size() << "  OSME " << averOSME << endl;
   totalNum = 0;
   int countttt = 0;
   ...
   cerr << averForward << " "<< countttt << " " << (mean1/(countttt)) << " " << (mean2/(countttt))  << endl;
   ...
   averForward = (double) totalNum/runtime.size();
   //cout << averForward <<endl;
}
...
void ExtractAlgo(string algo) {//
   ...
   while(getline(file,line)){
	      if(isdigit(line[0])) {
		      int count = 0, forward_, backward_,OSME_, chi_,t1,t2,t3,t4,t5,t6;
		      string mapNum, mapNa, width, height, startX,startY,goalX,goalY;
		      double pathCost,times;	  
		      stringstream strr(line);//>> num >> mn
		      //strr >> times >> forward_ >> backward_ >> count;
              strr >> mapNum >> mapNa>> width>> height>>startX >> startY >> goalX >> goalY >> pathCost>> forward_ >> backward_>> times >> OSME_>> chi_>> t1 >>t2 >> t3 >> t4>>t5>>t6; 
              optimal_ALGO.push_back(pathCost); 
              //cerr << times << endl;
              forwardE.push_back(forward_ + backward_);
              runtime.push_back(times);
              OSME.push_back(OSME_);
              Chi.push_back(chi_);
              T1.push_back(t1);
              T2.push_back(t2);
              T3.push_back(t3);
  ...
  file.close();
  cerr << forwardE.size() << " " << T1.size() << " " << T2.size() << " " << T3.size()<< " " << T4.size()<< " " << T5.size()<< " " << T6.size() << " " << runtime.size() << endl;
  double avg1 = 0, avg2 = 0 , avg3 = 0, avrtime_ =0, avg4 =0, avg5 = 0, avg6 = 0;
  double avgt1 = 0, avgt2 = 0 , avgt3 = 0, avgt4 =0, avgt5 = 0, avgt6 = 0;
  ...
  cerr << "rumtime-> " << avruntime_  << "  " << ct1_ << " " << ct2_ << " " << ct3_ << " " << ct4_ << " " << ct5_ << " " << ct6_ << endl;
...
int main(int argc, char ** argv) {
    string astar = argv[1], algo = argv[2];//, ral = argv[3]
    ExtractAstar(astar);
    ExtractAlgo(algo);
    //ExtractRAstar(ral);
    //averageCost();	 
